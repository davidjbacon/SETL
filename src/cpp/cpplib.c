...

#define SKIP_WHITE_SPACE(p) do { while (is_hor_space[(U_CHAR)*p]) p++; } while (0)
#define SKIP_ALL_WHITE_SPACE(p) do { while (is_space[(U_CHAR)*p]) p++; } while (0)
...
struct directive {
  ...
  int (*func)(cpp_reader *, struct directive *, U_CHAR *, U_CHAR *);
     ...
     const U_CHAR *str;
{
  ...
  if (*p == 0)
    {
      buf2 = (U_CHAR *) alloca (p - buf + 4);
      strcpy ((char *) buf2, (const char *) str);
      strcat ((char *) buf2, " 1");
      buf = buf2;
    }
  else if (*p != '=')
  ...
  else
    {
      ...
      buf2 = (U_CHAR *) alloca (2 * strlen ((const char *)str) + 1);
      strncpy ((char *) buf2, (const char *)str, p - str);
     ...
     const U_CHAR *str;
{
  ...
  *q = 0;
  ...
  cpp_push_buffer (pfile, buf, strlen ((const char *)buf));
     ...
     struct cpp_options *opts;
{
  ...
  /* Initialize is_idchar NOT to allow $.  */
...
/* Assuming we have read '$', and that "lang_setl" is in effect.
   This is the start of a SETL comment.  Skip to its end.  */

static void
skip_setl_comment (pfile, linep)
     cpp_reader *pfile;
     long *linep;
{
  int c;
  for (;;)
    {
      c = GETC ();
      if (c == EOF)
        return; /* Allow comment to be terminated by EOF. */
      while (c == '\\' && PEEKC() == '\n')
        {
          FORWARD(1);
          c = GETC();
...
/* Assuming we have read '-', and that "lang_setl" is in effect.
   If this is the start of a SETL2 comment (followed by another '-'),
   skip to the end of the comment, and return ' '.
   If not the start of a comment, return '-'. */

static int
skip_setl2_comment (pfile, linep)
     cpp_reader *pfile;
     long *linep;
{
  int c;
  ...
  while (1)
    {
      ...
      else if (c == '$' && CPP_OPTIONS (pfile)->lang_setl)
        {
          FORWARD (1);
          skip_setl_comment (pfile, NULL);
        }
      else if (c == '-' && CPP_OPTIONS (pfile)->lang_setl)
        {
          FORWARD (1);
          c = skip_setl2_comment (pfile, NULL);
          if (c == '-') {
            FORWARD(-1);
            return;
          }
...
{ int c;
  ...
  if (ident_length == 0 || !is_idstart[*ident]) {
    ...
    if (p == ident) {
      ...
      if (*p == '\n') {
        if (pedantic && !lang_asm && !lang_setl)
          cpp_warning (pfile, "invalid preprocessor directive");
        return 0;
      }
    }

    if (!lang_asm && !lang_setl)
  ...
  for (kt = directive_table; ; kt++) {
    ...
    if (kt->length == ident_length && !strncmp ((const char *)kt->name, (const char *)ident, ident_length))
      break;
  }
     ...
     struct directive *keyword;
{
  ...
  CPP_RESERVE (pfile, (int)(1 + keyword_length + (limit - buf)));
  CPP_PUTC_Q (pfile, '#');
     ...
     int predefinition;
{
  ...
  int rest_args = 0;
  long line, col;
  const char *file = CPP_BUFFER (pfile) ? CPP_BUFFER (pfile)->nominal_fname : "";
  DEFINITION *defn;
  ...
  if (*bp == '(') {
    ...
    while (*bp != ')') {
      ...
      while (is_idchar[*bp]) {
        ...
        if ((U_CHAR *) limit - bp > (ptrdiff_t)REST_EXTENSION_LENGTH &&
            strncmp ((const char *)rest_extension, (const char *)bp, REST_EXTENSION_LENGTH) == 0) {
          rest_args = 1;
          temp->rest_args = 1;
          break;
        }
    ...
    defn = collect_expansion (pfile, bp, (U_CHAR *) limit, argno, arg_ptrs);
    ...
    /* now everything from bp before limit is the definition. */
    defn = collect_expansion (pfile, bp, (U_CHAR *) limit, -1, NULL_PTR);
/* 'argnames' may get freed, so this code was highly suspect:
    defn->args.argnames = (U_CHAR *) "";  */
    defn->args.argnames = (U_CHAR *) xmalloc (1);
    defn->args.argnames[0] = '\0';
  }
...
/* Check a purported macro name SYMNAME, and yield its length.
   USE is the kind of name this is intended for.  */

static int
check_macro_name (pfile, symname, use)
     cpp_reader *pfile;
     ...
     const char *use;
{
  ...
  if (sym_length == 0)
    cpp_error (pfile, "invalid %s name", use);
  else if (!is_idstart[*symname]) {
    U_CHAR *msg;                        /* what pain... */
    msg = (U_CHAR *) alloca (sym_length + 1);
    bcopy (symname, msg, sym_length);
    msg[sym_length] = 0;
    cpp_error (pfile, "invalid %s name `%s'", use, msg);
  } else {
    if (! strncmp ((const char *)symname, "defined", 7) && sym_length == 7)
      cpp_error (pfile, "invalid %s name `defined'", use);
  }
  return sym_length;
}
     ...
     DEFINITION *d1, *d2;
{
  ...
  if (d1->nargs != d2->nargs)
    return 1;
  if (strcmp ((const char *)d1->args.argnames, (const char *)d2->args.argnames))
    return 1;
     ...
     enum file_change_code file_change;
{
  ...
  CPP_RESERVE (pfile, (int)(4 * strlen (ip->nominal_fname) + 50));
  {
     ...
     cpp_reader *pfile;
{
  ...
  switch (hp->type)
    ...
    case T_BASE_FILE:
      {
        ...
        if (!string)
          string = "";
        CPP_RESERVE (pfile, (int)(3 + 4 * strlen (string)));
    ...
    case T_INCLUDE_LEVEL:
      ...
      bif = (char *) alloca (8); /* Eight bytes ought to be more than enough */
      sprintf (bif, "%d", true_indepth - 1);
      buf = bif;
      break;

    case T_VERSION:
      bif = (char *) alloca (3 + strlen (version_string));
      sprintf (bif, "\"%s\"", version_string);
      buf = bif;
      break;
    ...
    case T_CONST:
      bif = (char *) alloca (4 * sizeof (int));
      sprintf (bif, "%d", hp->value.ival);
      buf = bif;
#if 0
    ...
    case T_TIME:
      ...
      if (hp->type == T_DATE)
        sprintf (bif, "\"%s %2d %4d\"", monthnames[timebuf->tm_mon],
                 timebuf->tm_mday, timebuf->tm_year + 1900);
      else
        sprintf (bif, "\"%02d:%02d:%02d\"", timebuf->tm_hour, timebuf->tm_min,
                 timebuf->tm_sec);
      buf = bif;
      break;
...
initialize_builtins (pfile)
     cpp_reader *pfile;
{
  install ((const U_CHAR *)"__LINE__", -1, T_SPECLINE, 0, 0, -1);
  install ((const U_CHAR *)"__DATE__", -1, T_DATE, 0, 0, -1);
  install ((const U_CHAR *)"__FILE__", -1, T_FILE, 0, 0, -1);
  install ((const U_CHAR *)"__BASE_FILE__", -1, T_BASE_FILE, 0, 0, -1);
  install ((const U_CHAR *)"__INCLUDE_LEVEL__", -1, T_INCLUDE_LEVEL, 0, 0, -1);
  install ((const U_CHAR *)"__VERSION__", -1, T_VERSION, 0, 0, -1);
#ifndef NO_BUILTIN_SIZE_TYPE
  install ((const U_CHAR *)"__SIZE_TYPE__", -1, T_SIZE_TYPE, 0, 0, -1);
#endif
#ifndef NO_BUILTIN_PTRDIFF_TYPE
  install ((const U_CHAR *)"__PTRDIFF_TYPE__ ", -1, T_PTRDIFF_TYPE, 0, 0, -1);
#endif
  install ((const U_CHAR *)"__WCHAR_TYPE__", -1, T_WCHAR_TYPE, 0, 0, -1);
  install ((const U_CHAR *)"__USER_LABEL_PREFIX__", -1, T_USER_LABEL_PREFIX_TYPE, 0, 0, -1);
  install ((const U_CHAR *)"__REGISTER_PREFIX__", -1, T_REGISTER_PREFIX_TYPE, 0, 0, -1);
  install ((const U_CHAR *)"__TIME__", -1, T_TIME, 0, 0, -1);
  if (!CPP_TRADITIONAL (pfile))
    install ((const U_CHAR *)"__STDC__", -1, T_CONST, STDC_VALUE, 0, -1);
  if (CPP_OPTIONS (pfile)->objc)
    install ((const U_CHAR *)"__OBJC__", -1, T_CONST, 1, 0, -1);
/*  This is supplied using a -D by the compiler driver
    so that it is present only when truly compiling with GNU C.  */
/*  install ((const U_CHAR *)"__GNUC__", -1, T_CONST, 2, 0, -1);  */
...
/*** There is a clear bug here, which you can tickle by turning on
 *** the -g3 option (for "debugging"; it produces a segfault):
 *** when STATIC_BUFFERS is asserted (which it is),
 *** CPP_PREV_BUFFER(pbuffer) just means pbuffer+1, so this loop
 *** just increments pbuffer by sizeof *pbuffer (which is 68 on a
 *** 32-bit platform) until just before it wraps around to 0.
 *** I am fixing this to test against CPP_NULL_BUFFER(pfile), but
 *** I see several other examples in this file of loops where the
 *** test is just against plain NULL, so there are probably other
 *** ways to get incorrect behaviour from this version of cpp too:  ***/
/***  while (CPP_PREV_BUFFER (pbuffer))   <--- code was   ***/
     ...
     HASHNODE *hp;
{
  ...
  if (nargs <= 0)
    ...
    {
      register U_CHAR *expansion = defn->expansion;
      register int offset;      /* offset in expansion,
                                   copied a piece at a time */
      register int totlen;      /* total amount of expansion buffer filled so far */
          ...
          if (ap->stringify != 0)
          ...
          else
            {
              ...
              if (!ap->raw_after && totlen > 0 && offset < defn->length
                  && !CPP_TRADITIONAL(pfile)
                  && unsafe_chars (xbuf[totlen-1], expansion[offset]))
                {
                  xbuf[totlen++] = '@';
                  xbuf[totlen++] = ' ';
                }
          ...
          if (pcf != -1)
            {
              ...
              fstat (pcf, &s);
              if (bcmp ((const char *) &stat_f.st_ino, (const char *) &s.st_ino,
                        sizeof (s.st_ino))
     ...
     struct directive *keyword;
{
  ...
  new_lineno = atoi ((const char *)pfile->token_buffer + old_written) - 1;
  CPP_SET_WRITTEN (pfile, old_written);
  ...
  if (token == CPP_STRING) {
    ...
    for (hp = *hash_bucket; hp != NULL; hp = hp->next)
      if (hp->length == fname_length &&
          strncmp ((const char *)hp->value.cpval, (const char *)fname, fname_length) == 0) {
     ...
     const char *buf, *limit;
{
  ...
  sym_length = check_macro_name (pfile, (const U_CHAR *) buf, "macro");

  while ((hp = cpp_lookup (pfile, (const U_CHAR *) buf, sym_length, -1)) != NULL)
    {
...
{
  cpp_buffer *ip = NULL;
  struct file_name_list *new_list;
  ...
  new_list = (struct file_name_list *) xmalloc (sizeof *new_list);
  new_list->next = pfile->dont_repeat_files;
  pfile->dont_repeat_files = new_list;
  new_list->fname = savestring (ip->fname);
  new_list->control_macro = 0;
  new_list->got_name_map = 0;
  new_list->c_system_include_path = 0;
     ...
     U_CHAR *buf, *limit;
{
  ...
  if (!strncmp ((const char *)buf, "implementation", 14)) {
    ...
    fname_len = p != NULL ? p - fname : (int) strlen ((const char *)fname);
     ...
     int length;
{
  ...
  save_defined = install ((const U_CHAR *)"defined", -1, T_SPEC_DEFINED, 0, 0, -1);
     ...
     int any;
{
  ...
  if (CPP_OPTIONS (pfile)->output_conditionals) {
    ...
    CPP_PUTS (pfile, failed, (int) sizeof failed - 1);
    pfile->lineno++;
    output_line_command (pfile, 1, same_file);
  }
...
GCC_DIAG_ON(implicit-fallthrough)
  ...
  for (;;) {
    switch (c)
      ...
      case '$':
        skip_setl_comment (pfile, NULL);
        break;
      case '-':         /* possible SETL2 comment */
        if (CPP_OPTIONS (pfile)->lang_setl) {
          (void) skip_setl2_comment (pfile, NULL);
        }
        break;
      case '\"':
      case '\'':
  ...
  if (CPP_OPTIONS (pfile)->output_conditionals) {
    ...
    CPP_PUTS (pfile, end_failed, (int) sizeof end_failed - 1);
    pfile->lineno++;
  }
...
/*                    cpp_pedwarn_with_line (pfile, start_line, start_column,
              ...
              if (CPP_OPTIONS (pfile)->pedantic_errors)
                cpp_error_with_line (pfile, start_line, start_column, "string constant runs past end of line");
              else
                cpp_warning_with_line (pfile, start_line, start_column, "string constant runs past end of line");
                    }
        ...
        case '$':
          if (opts->lang_setl) {
            ...
            cpp_buf_line_and_col (cpp_file_buffer (pfile),
                                  &start_line, &start_column);
            skip_setl_comment (pfile, &newlines);
            c = '$';  /* Initial letter of comment. */
            goto return_comment;
          }
          ...
          if (c2 == '-' && opts->lang_setl)
            {
              ...
              cpp_buf_line_and_col (cpp_file_buffer (pfile),
                                    &start_line, &start_column);
              c = skip_setl2_comment (pfile, &newlines);
              assert (c == ' ');
              c = '-';  /* Initial letter of comment. */
              goto return_comment;
            }
        ...
        letter:
          {
            ...
            if (pfile->buffer->rlimit - pfile->buffer->cur >= 3
              ...
              {
                int c4 = pfile->buffer->rlimit[-3];
                int c5 = CPP_BUF_PEEK (CPP_PREV_BUFFER (CPP_BUFFER (pfile)));
                if (c5 == EOF || ! unsafe_chars (c4, c5))
                  pfile->buffer->rlimit -= 2;
              }
          }
          goto get_next;
     ...
     struct file_name_list *searchptr;
{
  ...
  for (h = 0; h < IMPORT_HASH_SIZE; h++) {
    ...
    while (i) {
      ...
      if (!bcmp ((const char *) &i->inode, (const char *) &sb.st_ino, sizeof (sb.st_ino))
          && i->dev == sb.st_dev) {
        close (fd);
        return -2;              /* return found */
      }
     ...
     char *fname;
{
  ...
  strcpy (i->name, fname);
  bcopy ((const char *) &sb.st_ino, (char *) &i->inode, sizeof (sb.st_ino));
  i->dev = sb.st_dev;
     ...
     struct file_name_list *searchptr;
{
  ...
  if (searchptr
      && searchptr->fname
      && (ptrdiff_t) strlen (searchptr->fname) == p - filename
  ...
  for (map = read_name_map (pfile, cdir); map; map = map->map_next)
    if (! strcmp (map->map_from, from))
      return open (map->map_to, O_RDONLY, 0666);
     ...
     const char *fname;
{
  ...
  /* Some people say that CPATH should replace the standard include dirs,
     ...
     anyway.  */
  cpath = (const char *) getenv ("CPATH");
  if (cpath != 0 && ! opts->no_standard_includes)
    path_include (pfile, cpath);
    ...
    while (*p) {
      ...
      if (p[0] == '-' && p[1] == 'D') {
        ...
        if (opts->debug_output)
          output_line_command (pfile, 0, same_file);
        cpp_define (pfile, (const U_CHAR *) q);
        while (*p == ' ' || *p == '\t')
          p++;
      } else if (p[0] == '-' && p[1] == 'A') {
        ...
        *termination = '\0';
        ...
        make_assertion (pfile, "-A", (const U_CHAR *) assertion);
      ...
      if (pend->cmd != NULL && pend->cmd[0] == '-')
        {
          switch (pend->cmd[1])
            ...
            case 'D':
              if (opts->debug_output)
                output_line_command (pfile, 0, same_file);
              cpp_define (pfile, (const U_CHAR *) pend->arg);
              break;
            case 'A':
              make_assertion (pfile, "-A", (const U_CHAR *) pend->arg);
              break;
            }
        }
    }
  ...
  { /* read the appropriate environment variable and if it exists
    ...
    char *epath = 0;
    if (opts->lang_setl)
      epath = getenv ("SETL_INCLUDE_PATH");
    else
    ...
    if (epath) {
      ...
      bcopy ((const char *) include_defaults_array,
...
/*** a watchful newer gcc made me realize that strlen (default_prefix) is
 *** only 2 with my GCC_INCLUDE_DIR of "" (a pair of double quotes), or
 *** possibly 0 if the shell takes even those away first, so
 *** subtracting 7 or 8 from that size_t gives a huge positive number,
 *** and even when it gets put in int default_len, it gets blown up
 *** again when passed to strncmp() below, making it a useless bound.
 *** It's actually dangerous at the first strcmp() below.
 *** So, now guarding that strcmp, and default_len can remain 0 if
 *** default_prefix is too short to have an /include suffix.  ***/
    char *default_prefix = savestring (GCC_INCLUDE_DIR);
    int default_len = 0;
    /* Remove the `include' from /usr/local/lib/gcc.../include.  */
    if (strlen (default_prefix) >= 8 &&
       !strcmp (default_prefix + strlen (default_prefix) - 8, "/include")) {
      ...
      for (p = include_defaults; p->fname; p++) {
        ...
        if (!p->cplusplus
          ...
          if (!strncmp (p->fname, default_prefix, default_len)) {
            ...
            struct file_name_list *new_list
              = (struct file_name_list *) xmalloc (sizeof *new_list);
            ...
            new_list->fname = str;
            new_list->control_macro = 0;
            new_list->c_system_include_path = !p->cxx_aware;
            new_list->got_name_map = 0;
            append_include_chain (pfile, new_list, new_list);
            if (opts->first_system_include == 0)
              opts->first_system_include = new_list;
          }
        }
      }
...
/*** the pfile arg here was an interesting mistake:
  ...
  if (opts->print_deps)
    {
      ...
      if (opts->deps_target)
      ...
      else
        {
          ...
          if ((r = rindex (opts->in_fname, '/')) != NULL
#ifdef DIR_SEPARATOR
              && (r = rindex (opts->in_fname, DIR_SEPARATOR)) != NULL
#endif
              )
            ++r;
          else
     ...
     char **argv;
{
  ...
  for (i = 0; i < argc; i++) {
    if (argv[i][0] != '-') {
      ...
      case '-':
        if      (!strcmp (argv[i], "--help"))    opts->help    = 1;
        else if (!strcmp (argv[i], "--version")) opts->version = 1;
        else fatal ("Unrecognized `--' option: \"%s\"", argv[i]);
        break;
...
#endif
        if (! strcmp (argv[i], "-lang-setl"))
          opts->lang_setl = 1, opts->dollars_in_ident = 0;
        break;
      ...
      case '$':                 /* Don't include $ in identifiers.  */
        opts->dollars_in_ident = 0;
        /* There is in fact no user-accessible way to allow $ in
           identifiers in this particular adaptation of GNU CPP.  */
        break;
...
reverse_token_list (tokens)
     struct arglist *tokens;
{
  register struct arglist *prev = 0, *cur, *next;
  ...
  while (depth > 0)
    {
      ...
      bcopy ((const char *) (pfile->token_buffer + name_written),
     ...
     struct parse_marker *pmark;
{
  ...
  for (; ; pp = &(*pp)->next) {
    if (*pp == NULL) fatal ("internal error %s", "in parse_set_mark");
    if (*pp == pmark) break;
  }
  *pp = pmark->next;
}
...
void cpp_error (cpp_reader *pfile, const char *fmt, ...)
{
...
void cpp_warning (cpp_reader *pfile, const char *fmt, ...)
{
...
void cpp_pedwarn (cpp_reader *pfile, const char *fmt, ...)
{
  int is_error = 1;
  if (!CPP_OPTIONS (pfile)->pedantic_errors)
  {
...
void cpp_error_with_line (cpp_reader *pfile, int line, int column, const char *fmt, ...)
{
  cpp_buffer *ip = cpp_file_buffer (pfile);
...
void cpp_warning_with_line (cpp_reader *pfile, int line, int column, const char *fmt, ...)
{
  cpp_buffer *ip;
...
extern void cpp_pedwarn_with_file_and_line (cpp_reader *pfile, const char *file, int line, const char *fmt, ...)
{
  if (!CPP_OPTIONS (pfile)->pedantic_errors
      && CPP_OPTIONS (pfile)->inhibit_warnings)
    return;
  ...
  va_list args;
  va_start (args, fmt);
  cpp_vmessage (pfile, CPP_OPTIONS (pfile)->pedantic_errors, fmt, args);
  va_end (args);
  }
}
...
#ifndef VMS
#ifndef HAVE_STRERROR
  result = (const char *) ((errnum < sys_nerr) ? sys_errlist[errnum] : 0);
#else
...
static void usage(void) {
  ...
  printf("\n"
   ...
   "Directory names must be separated by a \"%c\" character.\n",
                                             PATH_SEPARATOR);
  printf("\n"
   "Other environment variables are as for GNU CPP version %s.\n",
                                                   version_string);
  printf("\n"
...
static void version(FILE *stream) {
  fprintf(stream, "GNU SETL %s preprocessor, based on GNU CPP %s\n",
