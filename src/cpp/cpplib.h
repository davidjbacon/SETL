...
Foundation, 58 Temple Place - Suite 330, Boston, MA 02111-1307, USA.

 In other words, you are welcome to use, share and improve this program.
 You are forbidden to forbid anyone else to use, share and improve
 what you give them.   Help stamp out software-hoarding!  */
/* Nice sentiment.  Pity about the spurious hyphen.  */
...
#ifndef PARAMS
#if defined(__STDC) || defined(__GNUC__)
#define PARAMS(P) P
#else
#define PARAMS(P) ()
#endif
...
/* Moved this enum out of scope of struct cpp_options:  */
...
enum dump_enum {dump_none = 0, dump_only, dump_names, dump_definitions};
...
struct cpp_options {
  ...
  /* Non-0 means print usage message and exit(0) immediately.  */
  char help;

  /* Non-0 means print version message and exit(0) immediately.  */
  char version;
  ...
  /* Nonzero means handle SETL program syntax ("--" and "$" comments,
     and non-cpp-directive lines that start with "#"; also, strings
...
/* Moved from scope of "struct definition" below:  */
struct reflist {
  struct reflist *next;
...
extern void cpp_define PARAMS ((cpp_reader*, const U_CHAR*));
extern HOST_WIDE_INT cpp_parse_expr PARAMS ((cpp_reader*));

extern void skip_rest_of_line (cpp_reader *pfile);
extern void cpp_error (cpp_reader *pfile, const char *fmt, ...) PRINTF_ATTRIBUTE(2, 3);
extern void cpp_warning (cpp_reader *pfile, const char *fmt, ...) PRINTF_ATTRIBUTE(2, 3);
extern void cpp_pedwarn (cpp_reader *pfile, const char *fmt, ...) PRINTF_ATTRIBUTE(2, 3);
extern void cpp_error_with_line (cpp_reader *pfile, int line, int column, const char *fmt, ...) PRINTF_ATTRIBUTE(4, 5);
extern void cpp_warning_with_line (cpp_reader *pfile, int line, int column, const char *fmt, ...) PRINTF_ATTRIBUTE(4, 5);
extern void cpp_pedwarn_with_line (cpp_reader *pfile, int line, int column, const char *fmt, ...) PRINTF_ATTRIBUTE(4, 5);
extern void cpp_pedwarn_with_file_and_line (cpp_reader *pfile, const char *file, int line, const char *fmt, ...) PRINTF_ATTRIBUTE(4, 5);
extern int cpp_read_check_assertion (cpp_reader *pfile);

extern void cpp_print_containing_files (cpp_reader *pfile);
extern void cpp_file_line_for_message (cpp_reader *pfile, const char *filename, int line, int column);
extern void cpp_message (cpp_reader *pfile, int is_error, const char *fmt, ...)
  PRINTF_ATTRIBUTE(3, 4);
extern void cpp_vmessage (cpp_reader *pfile, int is_error, const char *fmt, va_list args)
  PRINTF_ATTRIBUTE(3, 0);
extern void fatal (const char *fmt, ...) LIKE_PRINTF NO_RETURN;
extern void cpp_error_from_errno (cpp_reader *pfile, const char *name);
extern void cpp_perror_with_name (cpp_reader *pfile, const char *name);
extern void cpp_pfatal_with_name (cpp_reader *pfile, const char *name) NO_RETURN;

extern void init_parse_options (struct cpp_options *opts);
extern void init_parse_file (cpp_reader *pfile);
extern int push_parse_file (cpp_reader *pfile, const char *fname);
extern int parse_name (cpp_reader *pfile, int c);
extern void cpp_finish (cpp_reader *pfile);
