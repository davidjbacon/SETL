...
extern void *xmalloc PARAMS ((unsigned));
extern void *xrealloc PARAMS ((void *, unsigned));
...
static void integer_overflow (cpp_reader *pfile);
static long left_shift (cpp_reader *pfile, long a, int unsignedp, unsigned long b);
static long right_shift (cpp_reader *pfile, long a, int unsignedp, unsigned long b);
...
static struct operation initialized_operation = {ERROR, 0, 0, 0, 0};

...
parse_number (pfile, start, olen)
     ...
     int olen;
{
  struct operation op = initialized_operation;  /* compiler sat. */
  const char *p = start;
  int c;
 ...
 retry:
  ...
  switch (token)
  {
    ...
    case CPP_HSPACE:   case CPP_COMMENT: 
      goto retry;
    case CPP_NUMBER:
      return parse_number (pfile, (const char *) tok_start, tok_end - tok_start);
    case CPP_STRING:
	...
	while (ptr < tok_end && ((c = *ptr++) != '\''))
	  {
	    if (c == '\\')
	      {
                /*
                 *  This original code gives "warning: dereferencing
                 *  type-punned pointer will break strict-aliasing
                 *  rules" under GCC:
                 *
                 *   c = cpp_parse_escape (pfile, (char **)&ptr);
                 *
                 *  The 3 statements below replace that line:
                 */
                char *p = (char *) ptr;
                c = cpp_parse_escape (pfile, &p);
                ptr = (U_CHAR *) p;
      ...
      if (top == limit)
	{
	  ...
	  if (stack != init_stack)
	  ...
	  else
	    {
	      ...
	      bcopy ((const char *) stack, (char *) new_stack, old_size);
	    }
	  stack = new_stack;
